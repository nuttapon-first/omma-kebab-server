...
func TestCreateTransactionFailure(t *testing.T) {
	...
	testCases := []testCases{
		...
		{
			Name: "Test price is less than zero",
			Body: map[string]interface{}{
				"menuId":           1,
				"branch":           "test",
				"transactionType":  "sales",
			...
			CodeExpected:    http.StatusBadRequest,
			MessageExpected: "invalid type and price",
		},
	}
	...
	for _, val := range testCases {
		t.Run(val.Name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodPost, "/transactions", bytes.NewBuffer(jsonStr))
			c.Request.Header.Set("Content-Type", "application/json; charset=utf-8")

			transactionHandler := &TransactionHandler{}
			r.POST("/transactions", NewTestHandler(transactionHandler.New)) // Call to a handler method
